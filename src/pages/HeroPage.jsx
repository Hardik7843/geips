// /* eslint-disable no-unused-vars */
// import React from "react";
// import students1 from "@/../public/Person/students1.png";
// const HeroPage = () => {
// const shortIntro =
//   "An autonomous institution established in the year 2021 with the strength of 10 students for paramedical course. The journey was begun with 2 faculty in the field of imparting nursing education and paramedical training. It was a 2 tier system of Authorization. All India Council for Paramedical & Vocational from New Delhi";
// const typesOfStreams = [
//   "Nursing Assistant",
//   "OT Technicians",
//   "Diploma in Medical Lab Technology (DMLT)",
//   "Diploma in Dialysis Technology",
//   "MRI/CT Scan Technician",
//   "X-ray Technician & and many more",
// ];

//   return (
//     <div className="text-black min-h-[60vh] scroll-mt-[80px] mt-30">
//       <div className="px-10 border w-full flex flex-col md:flex-row gap-x-4">
//         <div className="w-full md:w-fit relative aspect-video rounded-2xl border-2">
//           <img
//             src={students1}
//             alt="person"
//             className=" object-contain rounded-2xl"
//           />
//         </div>

//         <div className="w-full md:w-[3/5] border break-words text-justify">
//           <p className="w-full">{shortIntro}</p>
//           <div className="w-full flex flex-col items-start text-black">
//             <p>All types of Paramedical courses majorly</p>

//             <ol className="list-disc text-blue-600 space-y-2 pl-6">
//               {typesOfStreams.map((stream, index) => (
//                 <li key={index} className="">
//                   {stream}
//                 </li>
//               ))}
//             </ol>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default HeroPage;

/* eslint-disable no-unused-vars */
import React from "react";
import students1 from "@/../public/Person/students1.png";
import { motion } from "framer-motion";

const HeroPage = () => {
  const shortIntro =
    "An autonomous institution established in the year 2021 with the strength of 10 students for paramedical course. The journey was begun with 2 faculty in the field of imparting nursing education and paramedical training. It was a 2 tier system of Authorization. All India Council for Paramedical & Vocational from New Delhi";

  const typesOfStreams = [
    "Nursing Assistant",
    "OT Technicians",
    "Diploma in Medical Lab Technology (DMLT)",
    "Diploma in Dialysis Technology",
    "MRI/CT Scan Technician",
    "X-ray Technician & and many more",
  ];

  const listVariants = {
    hidden: {},
    visible: {
      transition: {
        staggerChildren: 0.2,
      },
    },
  };

  const itemVariants = {
    hidden: { opacity: 0, y: 30 },
    visible: { opacity: 1, y: 0 },
  };

  return (
    <div className="text-black min-h-[60vh] scroll-mt-[80px] mt-30">
      <div className="px-10 text-2xl font-extrabold font-serif">
        Global Educational Institute of Paramedical Sciences
      </div>
      <div className="px-10 border w-full flex flex-col md:flex-row gap-x-4">
        {/* Image - Slide from Left */}
        <motion.div
          initial={{ x: -100, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.8 }}
          className="w-full md:w-fit relative aspect-video rounded-2xl border-2"
        >
          <img
            src={students1}
            alt="person"
            className="object-contain rounded-2xl"
          />
        </motion.div>

        {/* Text and Streams - Slide from Right */}
        <motion.div
          initial={{ x: 100, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.8 }}
          className="w-full md:w-[3/5] border break-words text-justify"
        >
          <p className="w-full">{shortIntro}</p>

          <div className="w-full flex flex-col items-start text-black mt-4">
            <p>All types of Paramedical courses majorly</p>

            {/* Staggered List Animation */}
            <motion.ol
              className="list-disc text-blue-600 space-y-2 pl-6"
              initial="hidden"
              animate="visible"
              variants={listVariants}
            >
              {typesOfStreams.map((stream, index) => (
                <motion.li key={index} variants={itemVariants}>
                  {stream}
                </motion.li>
              ))}
            </motion.ol>
          </div>
        </motion.div>
      </div>
    </div>
  );
};

export default HeroPage;
